...
const sectorIcon = readFileSync(fileURLToPath(new URL("./SectorIcon.vue", import.meta.url)), "utf8");
...
describe("icon source policy", () => {
  it("uses Lucide for every game and player pictogram", () => {
    for (const source of [gameIcon, playerToken, sectorIcon, soundToggle]) {
