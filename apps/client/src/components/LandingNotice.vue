...
const meta = computed(() => {
  ...
  if (landed.type === "asset") {
    ...
    if (props.game.phase === "WAITING_FOR_PURCHASE") return { icon: "purchase", eyebrow: `${country.value?.continent ?? "Secteur"} · registre`, title, status: "Choix de concessions", tone: "action", description: `${active.value?.name ?? "Le consortium actif"} peut acheter jusqu’à six concessions encore disponibles dans le registre de ${country.value?.name ?? "ce monde"}. Les droits d’extraction de ${resource.value?.name ?? "la ressource"} seront calculés ensuite.` };
    ...
    return { icon: "resolved", eyebrow: `${country.value?.continent ?? "Secteur"} · case classique`, title, status: "Deux étapes résolues", tone: "owned", description: `Les achats dans le registre de ${country.value?.name ?? "ce monde"}, puis tous les droits de ${resource.value?.name ?? "la ressource"}, ont été traités.` };
  }
