...
import { ASSETS, COUNTRIES, LEVER_CARDS, RESOURCES, TREND_CARDS, roundCreditAmount } from "@richesses-espace/game";
...
const meta = computed(() => {
  ...
  if (landed.kind === "dividend") return { icon: "dividend", eyebrow: "Prime d’expédition", title: specialResource.value?.name ?? landed.name, status: props.game.phase === "WAITING_FOR_PAYMENT" ? "Droits à régler" : "Prime reçue", tone: "trend", description: `${active.value?.name ?? "Le consortium actif"} reçoit ${roundCreditAmount((props.game.lastRoll?.total ?? 0) * .5)} crédits (moitié arrondie de ${props.game.lastRoll?.total ?? 0}), puis règle les droits de ${specialResource.value?.name ?? "la ressource associée"}.` };
